/**
 * Mapa muscular interactivo — Método Saavedra 1x2x3
 * Silueta anatómica (polígonos adaptados de react-body-highlighter, MIT)
 * highlightMusclesByExercise(['Pectorales', 'Deltoides'])
 */
(function () {
    'use strict';

    const MUSCLE_ALIASES = {
        'Pectorales': 'muscle-pectoral-mayor',
        'Pectoral Mayor': 'muscle-pectoral-mayor',
        'Deltoides': 'muscle-deltoides',
        'Dorsal ancho': 'muscle-dorsal-ancho',
        'Dorsal Ancho': 'muscle-dorsal-ancho',
        'Trapecio': 'muscle-trapecio',
        'Bíceps braquial': 'muscle-biceps-braquial',
        'Biceps braquial': 'muscle-biceps-braquial',
        'Tríceps braquial': 'muscle-triceps-braquial',
        'Triceps braquial': 'muscle-triceps-braquial',
        'Recto del abdomen': 'muscle-recto-abdomen',
        'Recto Abdominal': 'muscle-recto-abdomen',
        'Oblicuos': 'muscle-oblicuos',
        'Erectores espinales': 'muscle-erectores-espinales',
        'Erectores de la Columna': 'muscle-erectores-espinales',
        'Cuádriceps': 'muscle-cuadriceps',
        'Cuadriceps': 'muscle-cuadriceps',
        'Isquiosurales': 'muscle-isquiosurales',
        'Glúteos': 'muscle-gluteos',
        'Gluteos': 'muscle-gluteos',
        'Aductores': 'muscle-aductores',
        'Tríceps sural': 'muscle-triceps-sural',
        'Triceps sural': 'muscle-triceps-sural'
    };

    const EXERCISE_MUSCLES = {
        'ex-1': ['Pectorales'], 'ex-2': ['Pectorales'], 'ex-3': ['Pectorales'],
        'ex-4': ['Pectorales', 'Deltoides'], 'ex-5': ['Pectorales', 'Deltoides'],
        'ex-6': ['Pectorales'], 'ex-7': ['Pectorales', 'Tríceps braquial'],
        'ex-8': ['Pectorales', 'Tríceps braquial'],
        'ex-9': ['Dorsal ancho', 'Bíceps braquial'], 'ex-10': ['Dorsal ancho', 'Trapecio'],
        'ex-11': ['Dorsal ancho'], 'ex-12': ['Dorsal ancho', 'Trapecio'],
        'ex-13': ['Dorsal ancho', 'Trapecio'], 'ex-14': ['Dorsal ancho'],
        'ex-15': ['Trapecio'],
        'ex-16': ['Deltoides'], 'ex-17': ['Deltoides'], 'ex-18': ['Deltoides'],
        'ex-19': ['Deltoides'], 'ex-20': ['Deltoides', 'Trapecio'],
        'ex-21': ['Bíceps braquial'], 'ex-22': ['Bíceps braquial'],
        'ex-23': ['Tríceps braquial'], 'ex-24': ['Tríceps braquial'],
        'ex-25': ['Cuádriceps'], 'ex-26': ['Cuádriceps'], 'ex-27': ['Cuádriceps'],
        'ex-28': ['Cuádriceps'], 'ex-29': ['Cuádriceps'], 'ex-30': ['Cuádriceps'],
        'ex-31': ['Isquiosurales'], 'ex-32': ['Isquiosurales'], 'ex-33': ['Isquiosurales'],
        'ex-34': ['Isquiosurales', 'Glúteos'], 'ex-35': ['Isquiosurales', 'Glúteos'],
        'ex-36': ['Isquiosurales'],
        'ex-37': ['Glúteos'], 'ex-38': ['Aductores'], 'ex-39': ['Glúteos'],
        'ex-40': ['Glúteos'], 'ex-41': ['Glúteos'],
        'ex-42': ['Tríceps sural'], 'ex-43': ['Tríceps sural'], 'ex-44': ['Tríceps sural'],
        'ex-45': ['Recto del abdomen'], 'ex-46': ['Recto del abdomen'],
        'ex-47': ['Recto del abdomen'], 'ex-48': ['Recto del abdomen'],
        'ex-49': ['Recto del abdomen'], 'ex-50': ['Recto del abdomen'],
        'ex-51': ['Recto del abdomen'],
        'ex-52': ['Oblicuos'], 'ex-53': ['Oblicuos'], 'ex-54': ['Oblicuos'],
        'ex-55': ['Oblicuos'], 'ex-56': ['Oblicuos'], 'ex-57': ['Oblicuos'],
        'ex-58': ['Erectores espinales'], 'ex-59': ['Erectores espinales'],
        'ex-60': ['Erectores espinales']
    };

    /** Vista frontal — muscleId null = silueta base */
    const ANTERIOR = [
        { muscleId: 'muscle-pectoral-mayor', points: [
        ...
        { muscleId: 'muscle-oblicuos', points: [
        ...
        { muscleId: 'muscle-recto-abdomen', points: [
        ...
        { muscleId: 'muscle-biceps-braquial', points: [
        ...
        { muscleId: 'muscle-triceps-braquial', points: [
        ...
        { muscleId: 'muscle-deltoides', points: [
        ...
        { muscleId: 'muscle-aductores', points: [
        ...
        { muscleId: 'muscle-cuadriceps', points: [
        ...
        { muscleId: 'muscle-triceps-sural', points: [
    ...
    const POSTERIOR = [
        ...
        { muscleId: 'muscle-trapecio', points: [
        ...
        { muscleId: 'muscle-dorsal-ancho', points: [
        ...
        { muscleId: 'muscle-erectores-espinales', points: [
        ...
        { muscleId: 'muscle-gluteos', points: [
        ...
        { muscleId: 'muscle-isquiosurales', points: [
    ...
    function muscleId(name) {
        return MUSCLE_ALIASES[name] || null;
    }

    function polygonTags(points) {
        return points.map(function (pt) {
            return '<polygon points="' + pt.trim() + '"/>';
        }).join('');
    }

    function renderBodyView(parts, labelX, labelText, translateX) {
        var baseHtml = '';
        var groups = {};

        parts.forEach(function (part) {
            if (part.muscleId) {
                if (!groups[part.muscleId]) groups[part.muscleId] = [];
                groups[part.muscleId] = groups[part.muscleId].concat(part.points);
            } else {
                baseHtml += polygonTags(part.points);
            }
        });

        var html = '<text x="' + labelX + '" y="18" text-anchor="middle" class="muscle-view-label">' + labelText + '</text>';
        html += '<g transform="translate(' + translateX + ', 24) scale(1.72)">';
        html += '<g class="body-base">' + baseHtml + '</g>';

        Object.keys(groups).forEach(function (id) {
            html += '<g class="muscle-group" data-muscle="' + id + '">' + polygonTags(groups[id]) + '</g>';
        });
    ...
    window.highlightMusclesByExercise = function (muscleGroupsArray) {
        var svg = document.getElementById('saavedra-muscle-svg');
        var legend = document.getElementById('muscle-map-legend');
        if (!svg) return;

        svg.querySelectorAll('.muscle-group.is-active').forEach(function (el) {
            el.classList.remove('is-active');
        });

        var names = [];
        var activated = {};
        (muscleGroupsArray || []).forEach(function (name) {
            var id = muscleId(name);
            if (!id || activated[id]) return;
            var els = svg.querySelectorAll('[data-muscle="' + id + '"]');
        ...
        if (legend) {
            legend.innerHTML = names.length
                ? names.map(function (n) {
                    return '<span class="muscle-legend-tag">' + n + '</span>';
                }).join('')
                : '<span class="muscle-legend-empty">Selecciona un ejercicio para ver el músculo objetivo</span>';
        }
    };

    window.getMusclesForExercise = function (exerciseId) {
        return EXERCISE_MUSCLES[exerciseId] ? EXERCISE_MUSCLES[exerciseId].slice() : [];
    };

    window.clearMuscleHighlight = function () {
        window.highlightMusclesByExercise([]);
    };

    function svgMarkup() {
        return '<svg id="saavedra-muscle-svg" viewBox="0 0 400 400" xmlns="http://www.w3.org/2000/svg" aria-label="Mapa muscular anatómico">' +
            '<defs>' +
            '<linearGradient id="neon-cyan" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#00FFFF"/><stop offset="100%" stop-color="#00AEEF"/></linearGradient>' +
            '<filter id="glow"><feGaussianBlur stdDeviation="2.5" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter>' +
            '</defs>' +
            renderBodyView(ANTERIOR, 88, 'VISTA FRONTAL', 12) +
            renderBodyView(POSTERIOR, 312, 'VISTA POSTERIOR', 236) +
            '</svg>';
    }

    window.initMuscleMap = function (containerId) {
        var container = document.getElementById(containerId || 'muscle-map-container');
        if (!container || container.dataset.initialized) return;
        container.innerHTML = svgMarkup();
        container.dataset.initialized = '1';
        window.clearMuscleHighlight();
    };

    document.addEventListener('DOMContentLoaded', function () {
        window.initMuscleMap('muscle-map-container');
    });
})();
